...
pub async fn notify_button_state(button: &mut GamepadButton, connection: &Connection) {
...
pub async fn buttons_task(buttons: &mut GamepadInputs, conn: &Connection) {
    let futures = [
        notify_button_state(&mut buttons.b, conn),
        notify_button_state(&mut buttons.a, conn),
        notify_button_state(&mut buttons.c, conn),
        notify_button_state(&mut buttons.d, conn),
        notify_button_state(&mut buttons.e, conn),
        notify_button_state(&mut buttons.f, conn),
    ];
...
impl GamepadButton {
    ...
    pub fn new(name: &'static str, input: Button, ble_handle: u16) -> Self {
